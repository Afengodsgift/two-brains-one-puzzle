...
function renderRotationOperator() {
  return `<div class="rot-list">${state.puzzle.pieces.map((p) => `
    <button type="button" class="rot-piece ${state.selectedId === p.id ? "selected" : ""}" data-id="${p.id}">
      <span class="rot-glyph" style="transform:rotate(${p.facing * 90}deg)">${escapeHtml(p.label)}</span>
      <span class="rot-dir">${DIRS[p.facing]}</span>
    </button>`).join("")}</div>
    <div class="rot-controls"><button class="btn" id="ccwBtn">↺ 90°</button><button class="btn primary" id="cwBtn">↻ 90°</button></div>`;
}
function renderRotationObserver() {
  return `<span class="eyebrow">TARGET FACINGS</span><div class="rot-list target">${state.puzzle.pieces.map((p) => `
    <div class="rot-piece locked"><span class="rot-glyph" style="transform:rotate(${p.targetFacing * 90}deg)">${escapeHtml(p.label)}</span><span class="rot-dir">${DIRS[p.targetFacing]}</span></div>`).join("")}</div>
    <span class="eyebrow" style="margin-top:16px">LIVE (their board)</span><div class="rot-list">${state.puzzle.pieces.map((p) => `
    <div class="rot-piece locked ${p.facing === p.targetFacing ? "matched" : ""}"><span class="rot-glyph" style="transform:rotate(${p.facing * 90}deg)">${escapeHtml(p.label)}</span><span class="rot-dir">${DIRS[p.facing]}</span></div>`).join("")}</div>`;
}
...
function renderGame() {
  ...
  if (p.type === "rotation") {
    app.innerHTML = `<main class="shell game rotation-theme"><header class="topbar"><div class="brand">${escapeHtml(title)} · ${isOp ? "OPERATOR" : "OBSERVER"}</div><div class="timer ${state.remaining <= 15 ? "urgent" : ""}">${mm}:${ss}</div></header><p class="role-hint">${isOp ? "Tap a piece, then rotate. You do not see the targets." : "You see correct facings. Guide them. You cannot rotate."}</p>${isOp ? renderRotationOperator() : renderRotationObserver()}<p class="microcopy">${isOp ? "Describe pieces by shape." : "Tell them which way each should face."}</p></main>`;
    if (isOp) {
